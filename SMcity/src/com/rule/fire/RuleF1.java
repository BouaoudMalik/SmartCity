...
public class RuleF1 implements RuleI {
	...
	public boolean filter(ArrayList<EventI> matchedEvents, CorrelatorStateI c) {
		...
		if (matchedEvents.isEmpty()) {
			...
			if (SmartCityDescriptor.dependsUpon(position, id) && state.isLadderAvailable()) {
				return true;
			} else {
				return false;
			}
	...
	public void act(ArrayList<EventI> matchedEvents, CorrelatorStateI c) {
		...
		Serializable[] params = new Serializable[] { matchedEvents.get(0).getPropertyValue("position"),
				TypeOfFirefightingResource.HighLadderTruck };

		try {
			state.triggerFirstBuildingAlarm(state, params);
			System.out.println("[" + t + "] " + id + " " + type.toString() + " RuleF1 at " + position);
		} catch (Exception e) {
